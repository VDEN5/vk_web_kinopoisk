...
export function MovieList({ movies, showFavoriteButton = true }: MovieListProps): JSX.Element {
    ...
    return (
        <>
            <div className="list-group">
                {movies.map((movie) => (
                    movie.id && (
                        <div key={movie.id} className="list-group-item list-group-item-action d-flex align-items-center justify-content-between">
                            <Link
                            ...
                            >
                                ...
                                <div className="d-flex flex-column">
                                    ...
                                    <div className="d-flex gap-3 text-muted small">
                                        {movie.rating?.imdb && (
                                            <span>IMDb: {movie.rating.imdb}</span>
                                        )}
                                        {movie.year && (
                ...
                <Modal.Body>
                    {selectedMovie && (
                        <div className="d-flex align-items-center">
                            <img 
                                src={selectedMovie.poster?.url || PLACEHOLDER_IMAGE} 
                                alt={selectedMovie.name || selectedMovie.alternativeName}
                                style={{
                                    width: '50px',
                                    height: '75px',
                                    objectFit: 'cover',
                            ...
                            />
                            <div>
                                <h6>{selectedMovie.name || selectedMovie.alternativeName}</h6>
                                <p className="text-muted mb-0">
                                    {selectedMovie.year && `Год: ${selectedMovie.year}`}
                                    {selectedMovie.rating?.imdb && ` • IMDb: ${selectedMovie.rating.imdb}`}
                                </p>
                            </div>
                        </div>
                    )}
                    <p className="mt-3">Вы уверены, что хотите добавить этот фильм в избранное?</p>
                </Modal.Body>
                <Modal.Footer>
